...
class GameViewController: UIViewController, UICollisionBehaviorDelegate {
    ...
    lazy var carView: UIImageView = {
        ...
        carView.image = image
        carView.frame = CGRect(x: self.view.frame.size.width / CGFloat(2.1), y: self.view.frame.size.height / CGFloat(2), width: self.view.frame.size.width / CGFloat(5), height: self.view.frame.size.height / CGFloat(7))
    ...
    @IBAction func leftSwipeDetected() {
        UIView.animate(withDuration: 0.3, delay: 0.0, options: [.curveLinear], animations: {
            self.carView.frame.origin.x -= self.view.frame.size.width / CGFloat(6.5)
        }, completion: nil)
        roadsideExit()
    }
    ...
    func roadsideExit() {
        if carView.frame.origin.x < self.view.frame.size.width / CGFloat(6.5) || carView.frame.origin.x > 5 * self.view.frame.size.width / CGFloat(6.5) {
    ...
    func objectsFall() {
        var pointX = CGFloat(self.view.frame.size.width / CGFloat(6))
        ...
        for imageView in fallingObjectsArray {
            imageView.frame = CGRect(x: pointX, y: pointY,  width: self.view.frame.size.width / CGFloat(5), height: self.view.frame.size.height / CGFloat(7))
            pointX += self.view.frame.size.width / CGFloat(6.4)
    ...
    func startTimer() {
        timer = Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) {_ in
            for view in self.fallingObjectsArray {
                if self.carView.frame.origin.y == view.frame.origin.y + view.frame.size.height {
                    self.crashCarAppear()
                }
            }
        }
